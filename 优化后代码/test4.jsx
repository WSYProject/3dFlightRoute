...
class DataCount extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            compareData: [ // 用于临时计算各终点位置的参数
                { x: 0.65, y: 0.89, id:'1', name: '场站1' },
                { x: 0.094, y: 0.76, id:'2', name: '场站2' },
                { x: 0.95, y: 0.28, id:'3', name: '场站3' },
                { x: 0.19, y: 0.19, id:'4', name: '场站4' },
                { x: 0.49, y: 0.08, id:'5', name: '场站5' }
            ],
            radius: 20, // 航路点半径
            radiusRing: 1,
            radiusRingMin: 1,
    ...
    drawAllLine() {
        // 根据每个点分别画线
        this.state.canvas.clearRect(0, 0, document.getElementById('canvas').width, document.getElementById('canvas').height);
    ...
    drawLine({ x, y, id }) {
        ...
        // 线宽1
        this.state.canvas.lineWidth = this.state.selectedItem?.id === id ? 3 : 1;;
        // 线颜色
    ...
    drawPoint(x1, y1) {
        ...
        let than = this
        image.onload = () => {
            than.state.canvasPoint.drawImage(image, x1 - 10, y1 - 10, than.state.radius, than.state.radius);
        }
        image.src = iconWindActive;
    ...
    handleClick = (event) => {
        ...
        // 在此处检查点击位置是否在图像区域内
        // const {directionArr, center, radius} = this.state
        ([...this.state.directionArr, { ...this.state.center }]).forEach(point => {
            if (Math.abs(x - point.x) <= (this.state.radius / 2) && Math.abs(y - point.y) <= (this.state.radius) / 2) {
                console.log('Clicked on the image!', point, event.clientX, rect.left);
                let toolTipDiv = document.getElementById('tooltip')
                toolTipDiv.style.left = `${point.x - 28}px`
                toolTipDiv.style.top = `${point.y - 50}px`
                this.setState({selectedItem: point}, () => {
        ...
        return (
            <div className="box" >
                <div className="mapBox">
                    ...
                    <img className="airport" style={{ zIndex: '-1' }} id="airportIcon" src={iconWindActive} alt="" />
                    <div id="tooltip">
